...
class Dataset(data.Dataset):
    ...
    def __init__(self, args, split, transform):
        ...
        for annotation in annotations['annotations']:
            ...
            x1, y1, x2, y2 = float(bbox[0]), float(bbox[1]), float(bbox[0] + bbox[2]), float(bbox[1] + bbox[3])  # left top + right down
    ...
    def __getitem__(self, idx):
        ...
        return img, target  # img-(3,448,448)
    ...
    def encoder(self, boxes, labels):
        ...
        for i in range(cxcy.size()[0]):  # sign the label the object belong to
            cxcy_sample = cxcy[i]
