        ...
        for class_name in CAR_CLASSES:
            ...
            for d, image_id in enumerate(image_ids):
                ...
                if (image_id, class_name) in self.targets:
                    ...
                    fp[d] = 1
            ###################################################################
            # Please fill the codes to compute recall and precision
            ##################################################################
            fp = np.cumsum(fp)
            tp = np.cumsum(tp)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', default="./checkpoints/hku_mmdetector_best.pth", help='Pretrained Model Path')  # "./checkpoints/hku_mmdetector_best.pth"
    parser.add_argument('--output_file', default="./result.pkl", help='PKL for evaluation')  # "./result.pkl"
    parser.add_argument('--pos_threshold', default=0.1, type=float, help='Confidence threshold for positive prediction')    # 0.3
    parser.add_argument('--nms_threshold', default=0.5, type=float, help='Threshold for non maximum suppression')  #0.5
...
#  python eval.py --split 'val' --output_file "./result.pkl"
