...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, downsample=None):
        ...
        self.conv1 = redefine_conv3x3(in_planes, planes, stride)#定义卷积层
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, downsample=None):
        ...
        ##############################################################
        # Please define your layers with the BottleNeck from the paper "Deep Residual Learning for Image Recognition"
        #
        # Note: You **must not** use the nn.Conv2d here but use **redefine_conv3x3** and **redefine_conv1x1** in this script instead
        ##############################################################
        self.conv1 = redefine_conv1x1(in_planes, planes)#定义卷积层
        self.bn1 = nn.BatchNorm2d(planes)
    ...
    def forward(self, x):
        ...
        ##############################################################
        # Please write the forward function with your defined layers
        ##############################################################
        out = self.conv1(x)
        out = self.bn1(out)
        out = self.relu(out)
