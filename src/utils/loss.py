...
class yololoss(nn.Module):
    ...
    def forward(self, prediction, target):
        ...
        for i in range(0, box_target.size()[0], self.B):
            ...
            ###################################################################
            # Please fill the codes below to calculate the iou of the two boxes and substite the "?"
        ...
        ###################################################################
        # Please fill the codes below to calculate the location loss
        ##################################################################

        mid_point_loss = F.mse_loss(box_pred_response[:, :2], box_target_response[:, :2], reduction='sum')
        weight_height_loss = F.mse_loss(torch.sqrt(box_pred_response[:, 2:4]), torch.sqrt(box_target_response[:, 2:4]),
                                        reduction='sum')
        loc_loss = mid_point_loss + weight_height_loss
        ...
        ###################################################################
        # Please fill the codes below to calculate the Not Response Loss
        ##################################################################
